...
class ComidaYaApp:
    ...
    def realizar_registro(self):
        ...
        result = self.db.call_procedure('sp_registrar_usuario', 
                                       [nombre, apellido, email, telefono, password])
        
        if result and result[0].get('id_usuario'):
            ...
            messagebox.showerror("Error", result[0].get('mensaje', 'Error al registrar'))
    ...
    def mostrar_menu_cliente(self):
        ...
        tk.Button(header, text="🛒 Carrito ({})".format(len(self.carrito)), 
        ...
        tk.Button(header, text="🚪 Salir", font=('Arial', 12), 
                 bg='white', fg=self.color_primario, cursor='hand2',
                 command=self.mostrar_login).pack(side='right', padx=5, pady=20)
        ...
        # Búsqueda
        search_frame = tk.Frame(main_container, bg=self.color_blanco, relief='raised', bd=1)
        ...
        search_entry.insert(0, "Buscar productos...")
        
        # Categorías
        categorias_frame = tk.Frame(main_container, bg=self.color_fondo)
        categorias_frame.pack(fill='x', pady=(0, 20))
        ...
        tk.Button(categorias_frame, text="📋 Todo", font=('Arial', 11, 'bold'), 
                 ...
                 command=lambda: self.filtrar_productos(None)).pack(side='left', padx=5)
        
        for cat in categorias:
            tk.Button(categorias_frame, text=f"{cat['icono']} {cat['nombre']}", 
                     font=('Arial', 11), bg='white', cursor='hand2',
                     command=lambda c=cat['id_categoria']: self.filtrar_productos(c)).pack(side='left', padx=5)
        ...
        for producto in productos:
            self.crear_tarjeta_producto(scrollable_frame, producto, row, col)
            col += 1
            if col > 2:
                col = 0
                row += 1
